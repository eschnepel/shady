...
TASK-0011). (`ShadyClearDiagnosticSlotButton`, ADR-004 §2f, was replaced
by `switch.py`'s `ShadyFollowDiagnosticSlotSwitch` — ADR-004 §2g,
`TASK-0037-patch-4` — and is covered in `test_switch.py`.)
...
class TestAsyncSetupEntry:
    ...
    is added (one per config entry, not per string) — and no other
    button: the diagnosed-slot clear button is gone (ADR-004 §2g)."""
