...
to validate ADR-004 §1a's `DiagnosticMode` base class against needs
beyond the one mode ADR-004 actually specifies, and to capture the
design thinking now, while fresh, rather than re-deriving it whenever
this is eventually scheduled.
...
registry (one instance per mode name, ADR-004 §1a) and additional
`const.py` `DIAGNOSTIC_MODES` option strings — nothing else about the
select entity or the sensor dispatch changes. Concretely, sketched (not
committed) names: `"compare_regressions_daily"` and
`"compare_providers_daily"`.

The only shape difference from `CompareRegressionsMode` is **cardinality**
— **288 slots instead of 1.** ADR-004 §1a's `DiagnosticMode.compute()`
takes no parameter at all: a whole-day mode's `compute()` would resolve
its own 288 samples directly through its coordinator reference, the same
way `CompareRegressionsMode`'s `compute()` resolves its one diagnosed
sample — loop the day's 288 slots, pull each one's predicted-by-method
(or predicted-by-provider) and actual values via `self._coordinator`'s
public interface, and return one `DiagnosticSensorResult` per entity the
mode produces. This needs **no change to `diagnostics/base.py`**: there
is no shared cardinality-typed container to reuse or diverge from, only
the same "however many values `compute()` decides to gather" freedom
`CompareRegressionsMode` already has for its one slot, and no shared
type to leave a field `None` in — both sketched modes simply build
whatever per-slot shape they need internally, with no single "training
pool" to plot per slot the way one diagnosed slot has (ADR-004 §2).
...
`sensor_id` per compared provider. `DiagnosticResult`'s flat,
self-identifying `sensors: Sequence[DiagnosticSensorResult]` shape
(ADR-004 §1a) already accommodates either without string-index keying,
since neither mode varies over configured strings the way
`CompareRegressionsMode` does.
...
`compute()`. This is exactly why ADR-004 §1a keeps that function in
`aggregation.py` rather than moving it into
`diagnostics/compare_regressions.py`: a mode-independent, scope-
...
designed here. `DiagnosticResult` (ADR-004 §1a) already permits this: it
is an opaque `state`/`attributes` pair, not a scatter-specific contract —
a future mode's `compute()` can return whatever attribute shape suits a
day view without touching `diagnostics/base.py`.

---

## Consequences

- **Pro:** Validates that ADR-004 §1a's `DiagnosticMode` base class does
  not need to be revisited when either sketched mode is actually built —
  its zero-argument `compute()`, resolving whatever it needs through a
  held coordinator reference, already accommodates a 288-sample mode as
  naturally as a 1-sample one.
